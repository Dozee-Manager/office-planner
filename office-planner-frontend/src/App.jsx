...
function App(){
  ...
  async function addTask(e){
    e.preventDefault();
    if(!selectedEmployee || !taskDetails || !timeSpent) return alert("fill fields");
    const payload = {
    ...
    <div style={{ maxWidth: 900, margin: "20px auto", padding: 20, fontFamily: 'Arial, sans-serif' }}>
      ...
      <section style={{ border: "1px solid #ddd", padding: 12, marginBottom: 12 }}>
        ...
        <form onSubmit={addTask} style={{ display: 'grid', gap: 8 }}>
          ...
          <select value={selectedEmployee} onChange={e=>setSelectedEmployee(e.target.value)}>
            {employees.map(emp => <option value={emp.id} key={emp.id}>{emp.name}</option>)}
          </select>

          <label>Date</label>
          <DatePicker selected={taskDate} onChange={(d)=>setTaskDate(d)} dateFormat="yyyy-MM-dd" />
      ...
      <section style={{ border: "1px solid #ddd", padding: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
          ...
          <button onClick={()=>fetchTasks({ from: new Date().toISOString().slice(0,10) })}>Today</button>
        </div>
        <div style={{ overflowX: 'auto' }}>
        <table style={{ width: "100%", marginTop: 8, borderCollapse: 'collapse' }} border="1">
          <thead><tr><th>Date</th><th>Employee</th><th>Task</th><th>Hours</th></tr></thead>
          <tbody>
            {tasks.map(t => (
              <tr key={t.id}>
